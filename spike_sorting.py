...
def compile_wave_clus(subj_sess, proj_dir="/data7/goldmine", verbose=True):
    ...
    # Load LFP timestamps.
    lfp_timestamps, sr = load_lfp_timestamps(
        lfp_timestamps_f=lfp_timestamps_f, verbose=verbose
    )
        ...
        for chan in chans:
            ...
            if len(neurons) > 0:
                ...
                for iNeuron, neuron in enumerate(neurons):
                    ...
                    wave_clus["{}_ch{}_{}".format(roi, chan, iNeuron + 1)] = od(
                        [
                            ("spike_times", spike_times),  # ms
                            ("fr", fr),  # Hz
                            ("waveforms", waveforms[spike_inds_, :]),
                        ]
                    )  # spike x time
...
def load_lfp_timestamps(
    ...
    if lfp_timestamps_f is None:
        subj, sess = subj_sess.split("_")
        lfp_timestamps_f = op.join(
            proj_dir, "data", subj, sess, "micro_lfps", "lfpTimeStamps_micro.mat"
        )
    if op.exists(lfp_timestamps_f):
        ...
        if verbose:
            ...
            print(
                "{} timestamps over {} min and {:.1f} s".format(
                    len(lfp_timestamps), np.int(dur / 6e4), (dur % 6e4) * 1e-3
                )
            )
...
def load_montage(elec_montage_f, verbose=True):
...
def plot_isi_hist(
    ...
    if xscale == "log":
        ax.hist(
            isis, bins=10 ** np.linspace(0, 5, bins + 1), color=color, edgecolor=color
        )
        ax.set_xscale("log")
    elif xscale == "wave_clus":
        ax.hist(isis, bins=np.linspace(0, 1e2, bins + 1), color=color, edgecolor=color)
    elif xscale == "linear":
...
def plot_neuron3(
    ...
    if (fig is None) or (ax is None):
        ...
        ax = np.ravel(ax)
    if fontsize is None:
        fontsize = od([("tick", 12), ("label", 14), ("annot", 12), ("fig", 16)])
    ...
    i = 1
    ax[i] = plot_isi_hist(
        neuron_dat, ax=ax[i], xscale=isi_xscale, color=colors[0], fontsize=fontsize
    )

    i = 2
    ax[i] = plot_spike_stability(neuron_dat, session_dur_ms, ax[i], fontsize=fontsize)
...
def plot_spike_stability(
    ...
    ax.hist(
        spike_times,
        bins=np.linspace(0, session_dur_ms, 61),
        color=color,
        edgecolor=color,
    )
...
def plot_waveforms(
    ...
    if n_spikes > samp_spikes:
        waves = neuron_dat["waveforms"][
            np.linspace(0, n_spikes - 1, samp_spikes, dtype=np.int), :
        ]
    else:
        waves = neuron_dat["waveforms"]
    ...
    ax.plot(
        x_units, mean_waveform - std_waveform, color="k", linewidth=1.2, linestyle="--"
    )
    ax.plot(
        x_units, mean_waveform + std_waveform, color="k", linewidth=1.2, linestyle="--"
    )
