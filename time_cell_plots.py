...
def time_raster(
    ...
    **kws
    ...
    game_state_durs = od(
        {
            game_state: events_proc.get_game_state_durs()[game_state]
    ...
    for iTrial, trial in enumerate(events.keep_trials):
        ...
        for game_state in game_states:
            ...
            spike_times = (
                spikes["spike_times"][
                    (spikes["spike_times"] >= event_start)
                    & (spikes["spike_times"] < event_stop)
                ]
                - event_start
            )
    ...
    ax.set_xticklabels(
        np.arange(0, (v_lines[-1] / 1000) + 1, xtick_inc, dtype=np.int),
        fontsize=font["tick"],
        rotation=0,
    )
    ...
    if plot_game_states:
        ...
        for ii in range(len(annot_x)):
            ax.text(
                annot_x[ii] / v_lines[-1],
                1.1,
                game_states[ii].replace("Delay", "Delay "),
                ha="center",
                va="center",
                fontsize=font["annot"],
...
def firing_rate_over_time(
    ...
    **kws
    ...
    if overlap:
        v_lines = (
            np.array([0, np.max(list(game_state_durs.values()))]) / timebin_size
        ).astype(int)
    else:
        v_lines = (
            np.cumsum([0] + list(game_state_durs.values())) / timebin_size
        ).astype(int)
    ...
    spike_mat = [
        event_spikes.get_spike_mat(neuron, game_state).values * mult
        for game_state in game_states
    ]  # [trial x time_bin,]
    ...
    if smooth > 0:
        for ii in range(len(spike_mat)):
            spike_mat[ii] = np.array(
                [
                    gaussian_filter1d(spike_mat[ii][iTrial, :].astype(float), smooth)
    ...
    sem_frs = [
        stats.sem(_spike_mat, axis=0, nan_policy="omit") for _spike_mat in spike_mat
    ]
    ...
    for ii in range(len(game_states)):
        ...
        if plot_grand_mean:
            ax.plot(
                xvals,
                [grand_mean[ii]] * xvals.size,
                color=grand_mean_color,
                linewidth=linewidth,
                linestyle=grand_mean_linestyle,
            )
        ax.plot(
            xvals,
            mean_frs[ii],
...
def plot_firing_maze2(
    ...
    **kws
    ...
    events = events_proc.load_events(subj_sess, proj_dir=proj_dir, verbose=False)
    events.maze.maze.loc[:, "region_precise"] = events.maze.maze[
        "region_precise"
    ...
    _min = np.min(
        [
            fr_pos[game_state][np.isfinite(fr_pos[game_state])].min()
            for game_state in game_states
        ]
    )
    _max = np.max(
        [
            fr_pos[game_state][np.isfinite(fr_pos[game_state])].max()
            for game_state in game_states
        ]
    )
    ...
    for game_state in game_states:
        ...
        for trial in events.keep_trials:
            ...
            keep_spikes = np.any(
                events.event_times.query(qry)["time_bins"]
                .apply(lambda x: [x[0] <= spike < x[-1] for spike in spike_times])
                .tolist(),
                axis=0,
            )
            _spike_times = spike_times[keep_spikes]
            spike_pos_arr = np.array(
                _positions["start_time"]
                .apply(lambda x: [spike - x for spike in _spike_times])
                .tolist()
            )  # pos x spike
            spike_pos_arr[spike_pos_arr < 0] = 1e6
            _spike_loc = _positions.loc[np.argmin(spike_pos_arr, axis=0), "pos"].apply(
                lambda x: tuple(
                    [((x[_i] * scale_by) - _origin[_i]) for _i in range(len(x))]
                )
            )
            if only_show_spikes_when_moving:
                keep_spikes = _positions.loc[
                    np.argmin(spike_pos_arr, axis=0), "moved_pos"
                ]
        ...
        for idx, maze_row in events.maze.maze.query("(region=='wall')").iterrows():
            ...
            mesh_coords = tuple(
                np.meshgrid(
                    range(coords[0][0], coords[1][0]), range(coords[0][1], coords[1][1])
                )
            )
            mask[mesh_coords] = 1
        ...
        if draw_base:
            coords = (
                np.array(events.maze.maze.query("(region=='base')")["coords"].tolist())
                * scale_by
            ) - _origin
            ...
            rect = patches.Rectangle(
                base_min, base_width, base_height, fill=False, lw=base_lw, ec=base_color
            )
        ...
        if show_nav:
            ...
            all_pos = (
                events.positions.query("(gameState=='{}')".format(game_state))
                .groupby(["trial"])["pos"]
        ...
        if show_spikes:
            ax[iax].plot(
                ...
                ms=spike_markersize,
                mew=spike_mew,
                mfc=spike_fill_color,
                mec=spike_edge_color,
            )

        # Configure the colorbar.
        cbar = ax[iax].collections[0].colorbar
        cbar.ax.tick_params(
            labelsize=font["tick"], length=ticklength, width=tickwidth, pad=tickpad
        )
...
def plot_firing_maze_split_time(
    subj_sess,
    neuron,
    scale_by=10,
    draw_base=True,
    ...
    **kws
    ...
    # -------------------------------------------------
    # Get user-defined plot params.
    time_wins = kws.pop("time_wins", [(0, 20), (20, 40), (40, 60)])
    game_states = kws.pop("game_states", ["Encoding", "Retrieval"])
    colws = kws.pop("colws", {1: 2.05, 2: 3.125, 3: 6.45})
    dpi = kws.pop("dpi", 1200)
    font = kws.pop("font", {"tick": 5, "label": 6, "fig": 7})
    wall_cmap = kws.pop("wall_cmap", "binary")
    ...
    spike_markersize = kws.pop("spike_markersize", 0.75)
    spike_mew = kws.pop("spike_mew", 0.15)
    no_text = kws.pop("no_text", False)

    plt.close()
    if len(time_wins) == 2:
        if no_text:
            grid_shp = kws.pop("grid_shp", (50, 101))
            figsize = kws.pop(
                "figsize", (colws[1], colws[1] * (grid_shp[0] / grid_shp[1]))
            )
            fig = plt.figure(figsize=figsize, dpi=dpi)
            ax = [
                plt.subplot2grid(grid_shp, (0, 0), rowspan=50, colspan=50),
                plt.subplot2grid(grid_shp, (0, 51), rowspan=50, colspan=50),
            ]
        else:
            grid_shp = kws.pop("grid_shp", (50, 120))
            figsize = kws.pop(
                "figsize", (colws[1], colws[1] * (grid_shp[0] / grid_shp[1]))
            )
            fig = plt.figure(figsize=figsize, dpi=dpi)
    ...
    elif len(time_wins) == 3:
        if no_text:
            grid_shp = kws.pop("grid_shp", (50, 152))
            figsize = kws.pop(
            ...
            ax = [
                plt.subplot2grid(grid_shp, (0, 0), rowspan=50, colspan=50),
                plt.subplot2grid(grid_shp, (0, 51), rowspan=50, colspan=50),
                plt.subplot2grid(grid_shp, (0, 102), rowspan=50, colspan=50),
            ]
        else:
            grid_shp = kws.pop("grid_shp", (50, 185))
            figsize = kws.pop(
                "figsize", (colws[2], colws[2] * (grid_shp[0] / grid_shp[1]))
            )
            fig = plt.figure(figsize=figsize, dpi=dpi)
            ax = [
                ...
                plt.subplot2grid(grid_shp, (0, 66), rowspan=50, colspan=50),
                plt.subplot2grid(grid_shp, (0, 117), rowspan=50, colspan=3),
                plt.subplot2grid(grid_shp, (0, 132), rowspan=50, colspan=50),
                plt.subplot2grid(grid_shp, (0, 182), rowspan=50, colspan=3),
            ]
    ...
    for time_win in time_wins:
        fr_pos[time_win] = (
            event_spikes.event_spikes.query(
                "(gameState=={}) & ({}<=time_bin<{})".format(
                    game_states, time_win[0], time_win[1]
                )
            )
            .groupby("maze_region")[neuron]
            .mean()
    ...
    _min = np.min(
        [
            fr_pos[time_win][np.isfinite(fr_pos[time_win])].min()
            for time_win in time_wins
        ]
    )
    _max = np.max(
        [
            fr_pos[time_win][np.isfinite(fr_pos[time_win])].max()
            for time_win in time_wins
        ]
    )
    kws["vmin"] = kws.get("vmin", np.max((0, _min)))
    ...
    # Make the plot.
    for iTime, time_win in enumerate(time_wins):
        if no_text:
            iax = iTime
        else:
            iax = int(iTime * 2)
        ...
        for trial in events.keep_trials:
            ...
            keep_spikes = np.any(
                ...
                .apply(
                    lambda x: [
                        x[time_win[0]] <= spike < x[time_win[1]]
        ...
        if no_text:
            ax[iax] = sns.heatmap(
                maze_fr_mat.T, ax=ax[iax], cbar=False, cmap=fr_cmap, **kws
            )
        else:
            ax[iax] = sns.heatmap(
                maze_fr_mat.T,
        ...
        if show_nav:
            ...
            #     keys: event numbers
            #     values: (time, (xPos, yPos)) array of position recordings
            qry = "(gameState=={})".format(game_states)
            time_bounds = np.array(
                events.event_times.query(qry)["time_bins"]
                .apply(lambda x: (x[time_win[0]], x[time_win[1]]))
                .tolist()
            )
            pos_idx = np.where(
                events.positions.query(qry).apply(
                    lambda x: np.all(
                        (
                            np.any(
                                (x["start_time"] >= time_bounds[:, 0])
                                * (x["start_time"] < time_bounds[:, 1])
                            ),
                            np.any(
                                (x["stop_time"] >= time_bounds[:, 0])
                                * (x["stop_time"] < time_bounds[:, 1])
                            ),
                        )
                    ),
                    axis=1,
            ...
            pos_idx = events.positions.query(qry).iloc[pos_idx].index.tolist()
            all_pos = (
                events.positions.loc[pos_idx]
                .groupby(["trial", "gameState"])["pos"]
                .apply(lambda x: (np.array(x.tolist()) * scale_by) - _origin)
            )
            for event in all_pos.index:
                ax[iax].plot(
                    all_pos[event][:, 0],
                    all_pos[event][:, 1],
                    lw=nav_lw,
    ...
    **kws
    ...
    if "vmin" not in kws:
        if fr_pos.max() < 6:
            ...
            kws["vmin"] = np.max(
                (0, int(fr_pos.min() - (0.2 * (fr_pos.max() - fr_pos.min()))))
            )
    ...
    fig = plt.figure(figsize=figsize, dpi=dpi)
    ax = [
        plt.subplot2grid(grid_shp, (0, 0), rowspan=100, colspan=100),
    ...
    # Make firing rate by maze region heatmap.
    ax[iax] = sns.heatmap(
        maze_fr_mat.T, ax=ax[iax], cbar=True, cbar_ax=ax[iax + 1], cmap=fr_cmap, **kws
    )
    ...
    if show_nav:
        ...
        #     values: (time, (xPos, yPos)) array of position recordings
        all_pos = (
            events.positions.query("(gameState=={})".format(game_states))
            .groupby(["trial", "gameState"])["pos"]
            .apply(lambda x: (np.array(x.tolist()) * scale_by) - _origin)
        )
        for trial in all_pos.index:
...
def plot_firing_hd(
    ...
    **kws
    ...
    for trial in events.keep_trials:
        ...
        spike_pos_arr[spike_pos_arr < 0] = 1e6
        spike_rot += _positions.loc[
            np.argmin(spike_pos_arr, axis=0), "rotation"
        ].tolist()
    spike_rot = np.array(spike_rot)

    # How many spikes were recorded in each direction?
    spike_hd = np.unique(
        [events._head_direction(x, n_direc) for x in spike_rot], return_counts=True
    )

    # How many positions were recorded in each direction?
    all_hd = np.unique(
        [
            events._head_direction(x, n_direc)
            for x in events.positions.query(
                "(trial=={}) & (gameState=='{}')".format(events.keep_trials, game_state)
            )["rotation"]
        ],
        return_counts=True,
    )
    ...
    head_direc = (
        pd.concat(
            (
                pd.Series(all_hd[1], all_hd[0], name="pos_count"),
                pd.Series(spike_hd[1], spike_hd[0], name="spikes"),
            ),
            axis=1,
        )
        .reset_index()
        .rename(columns={"index": "hd"})
    )
    hd_angles = {
        "N": (2 * np.pi) / 4,
        "NE": (1 * np.pi) / 4,
    ...
    head_direc.insert(
        3, "pct_time", head_direc["pos_count"] / np.sum(head_direc["pos_count"])
    )
    ...
    if "vmin" in kws:
        ...
        vmin = np.max(
            (
                ...
                int(
                    head_direc["fr"].min()
                    - (0.2 * (head_direc["fr"].max() - head_direc["fr"].min()))
                ),
            )
        )
    vmax = kws.get(
    ...
    ax[iax].set_yticklabels(
        ["{:.1f}".format(x) for x in yticks], fontsize=font["tick"], color=grid_color
    )
    ...
    **kws
    ...
    for trial in events.keep_trials:
        ...
        for gold_id in gold_ids:
            _gold_events = events.gold_events.query(
                "(trial=={}) & (gameState=='{}') & (gold_id=='{}')".format(
                    trial, game_state, gold_id
                )
            )
            view_state = -1
            giv_on = []
    ...
    if plot_labels:
        ...
        ax.set_ylabel(
            "Gold view 1..{}".format(n_gold_views),
            fontsize=font["label"],
            labelpad=labelpad,
        )
    if plot_title:
        ax_title = "{}-{} ({}), {}".format(
            subj_sess, neuron, spikes["hemroi"], game_state
        )
        ax.set_title(ax_title, loc="left", pad=6, fontsize=font["fig"])
