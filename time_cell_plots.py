...
def plot_firing_maze_split_time(subj_sess,
    ...
    if len(time_wins) == 2:
        if no_text:
            ...
            figsize = kws.pop('figsize', (colws[1], colws[1] * (grid_shp[0]/grid_shp[1])))
            fig = plt.figure(figsize=figsize, dpi=dpi)
            ax = [plt.subplot2grid(grid_shp, (0, 0), rowspan=50, colspan=50),
        ...
        if no_text:
            ...
            figsize = kws.pop('figsize', (colws[2], colws[2] * (grid_shp[0]/grid_shp[1])))
            fig = plt.figure(figsize=figsize, dpi=dpi)
            ax = [plt.subplot2grid(grid_shp, (0, 0), rowspan=50, colspan=50),
                  plt.subplot2grid(grid_shp, (0, 51), rowspan=50, colspan=50),
    ...
    for time_win in time_wins:
        fr_pos[time_win] = (event_spikes.event_spikes
                            .query("(gameState=={}) & ({}<=time_bin<{})"
                                   .format(game_states, time_win[0], time_win[1]))
                            .groupby('maze_region')[neuron].mean() * 2)

    _min = np.min([fr_pos[time_win][np.isfinite(fr_pos[time_win])].min() for time_win in time_wins])
    _max = np.max([fr_pos[time_win][np.isfinite(fr_pos[time_win])].max() for time_win in time_wins])
    kws['vmin'] = kws.get('vmin', np.max((0, _min)))
    kws['vmax'] = kws.get('vmax', _max)
    ...
    for iTime, time_win in enumerate(time_wins):
        ...
        for trial in events.keep_trials:
            ...
            _positions = events.positions.query(qry).reset_index(drop=True)
            keep_spikes = np.any(events.event_times.query(qry)['time_bins']
                                                   .apply(lambda x: [x[time_win[0]]<=spike<x[time_win[1]]
        ...
        # Make firing rate by maze region heatmap.
        if no_text:
            ax[iax] = sns.heatmap(maze_fr_mat.T, ax=ax[iax], cbar=False, 
                                  cmap=fr_cmap, **kws)
        else:
            ax[iax] = sns.heatmap(maze_fr_mat.T, ax=ax[iax], cbar=True, 
                                  cbar_ax=ax[iax+1], cmap=fr_cmap, **kws)
        ...
        if show_nav:
            ...
            time_bounds = np.array(events.event_times
                                   .query(qry)['time_bins']
                                   .apply(lambda x: (x[time_win[0]], x[time_win[1]])).tolist())
            pos_idx = np.where(events.positions
                               .query(qry)
                               .apply(lambda x: np.all((np.any((x['start_time'] >= time_bounds[:, 0]) *
                                                               (x['start_time'] < time_bounds[:, 1])),
                                                        np.any((x['stop_time'] >= time_bounds[:, 0]) *
                                                               (x['stop_time'] < time_bounds[:, 1])))),
                                      axis=1))[0]
            pos_idx = events.positions.query(qry).iloc[pos_idx].index.tolist()
            all_pos = (events.positions
                       .loc[pos_idx]
                       .groupby(['trial', 'gameState'])['pos']
                       .apply(lambda x: (np.array(x.tolist()) * scale_by) - _origin))
            for event in all_pos.index:
                ax[iax].plot(all_pos[event][:, 0], 
                             all_pos[event][:, 1],
                             lw=nav_lw,
                             alpha=nav_alpha,
                             color=nav_color)
