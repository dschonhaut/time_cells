    ...
    for iPerm in range(n_perm):
        if smooth > 0:
            ...
            spike_mat_null = {game_state: np.array([_shift_spikes(spike_mat[game_state][iTrial, :])
                              ...
                              for game_state in game_states}
        for game_state in game_states:
            mean_frs_null[game_state].append(np.mean(spike_mat_null[game_state], axis=0)) # perm x time

    # Z-score firing rates.
    _null_mean = np.mean(mean_frs_null[game_state], axis=0)
    _null_std = np.std(mean_frs_null[game_state], axis=0)
    z_frs = {game_state: (mean_frs[game_state] - _null_mean) / _null_std
             for game_state in game_states}

    # Find max and min Z-scores for each permutation of the null distribution.
    z_frs_null = {game_state: (mean_frs_null[game_state] - _null_mean) / _null_std
                  for game_state in game_states} # perm x time
    z_frs_null_max = {game_state: np.max(z_frs_null[game_state], axis=1)
                      for game_state in game_states}
